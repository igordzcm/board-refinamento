...
- **The real Portal Retaguarda backend/frontend lives in a different Azure DevOps project**, "Projeto - Conciliação de Caixa", repos `ConciliaçãoCaixaAPI` / `ConciliaçãoCaixaFront" — and **both are already cloned locally**, one directory above `board-refinamento/`, at `TAREFAS/ConciliaçãoCaixaAPI` and `TAREFAS/ConciliaçãoCaixaFront`. Use `Glob`/`Grep`/`Read` directly on these local paths — faster and more reliable than the ADO code-search API, and this is where Motor de Descontos, Migração Var Ret, Automação Fiscal, and the rest of the Retaguarda modules actually live (`src/modules/<domain>`).
