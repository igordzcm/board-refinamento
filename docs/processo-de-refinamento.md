...
6. Separar achados técnicos como "levantamento — sugestão", nunca misturados no AC.
7. Registrar decisões e pendências como comentário no card (rastro auditável).
8. Marcar a tag `Refinamento: Pronto` / `Refinamento: Quase Pronto` / `Refinamento: Precisa Refinar` no work item.
9. Atualizar o board (`boards/board-refinamento.html`) se o status mudou.
