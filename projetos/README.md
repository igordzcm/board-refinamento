...
|---|---|---|---|
| Dashboard CDS | — | Reunião com a Maria (12/08) descartou a premissa de trocar de fonte/eliminar o FTP — ela não mudou o WMS. Prioridade agora é replicar no sistema 8 mudanças que ela já fez na planilha (data, tabela de lojas, gráfico, rotas unificadas, PROCV, lead time ponderado); #11973 (FTP) segue sem decisão | [dashboard-cds/contexto.md](dashboard-cds/contexto.md) |
| Conciliação Fase 2 | Semana de 10/08 (US8-12) | 90% — US8-12 em homologação técnica (In Test). Bug novo na parte de "linhas" reportado por Diego (12/08), Diego segue focado nas correções finais; Kauã libera de volta pra Gamificação. Próximo passo de negócio: US13 segue com apresentação ao Ozeias/validação com as áreas | [conciliacao-fase-2/contexto.md](conciliacao-fase-2/contexto.md) |
| Migração VARRet (lote 2) | — | 90%+ — **Lote 2 concluído pelo JB**, em code review/QA (Ozéias vai marcar call com JB pra subir tudo). Próxima tarefa do JB: migrar exportações Excel/PDF do lote 1 pro padrão worker+e-mail (card #12484), depois volta pro Dashboard CDs | [migracao-varret/contexto.md](migracao-varret/contexto.md) |
| Motor de Descontos | — | Maior parte do escopo priorizado entregue em 22/06 sob Kauã; falta só a US5 (notificação automática às lojas na vigência), com escopo pendente de call entre Igor e Arthur — mencionado de novo em 12/08 como "legalzinho", sem dono definido ainda | [motor-descontos/contexto.md](motor-descontos/contexto.md) |
...
| Gamificação (Roleta 3.0) | — | 40% — Kauã libera da Conciliação Fase 2 e volta em breve (combinado 12/08). Pendência nova: Ozéias não consegue mais ver o Portal Gamificação em homolog, precisa investigar antes do retomo. Cards T7-T10 (Epic #11950) seguem em Refinement com bloqueios externos (CRM/BI+pixel do Marketing, assets de design, ambiente/stakeholders) | [gamificacao/contexto.md](gamificacao/contexto.md) |
...
- [dailies/](dailies/) — cole as transcrições de daily (ou de reuniões transversais, tipo planejamento de sprint) aqui; usadas pra manter o dashboard, o `prioridades-sprint.md` e os `contexto.md` atualizados.
